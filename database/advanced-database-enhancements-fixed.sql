-- ===============================
-- 🚀 ADVANCED DATABASE ENHANCEMENTS (TRANSACTION-SAFE)
-- ShahinKSA Compliance Platform
-- ===============================

-- 1. PERFORMANCE ENHANCEMENTS (Non-concurrent indexes for transaction safety)
-- ===============================

-- Advanced Indexing Strategy (using regular CREATE INDEX for transaction compatibility)
CREATE INDEX IF NOT EXISTS idx_controls_unified_performance 
ON controls_unified USING btree (framework_code, category, status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_controls_unified_fulltext 
ON controls_unified USING gin (to_tsvector('english', title_en || ' ' || COALESCE(description_en, '')));

CREATE INDEX IF NOT EXISTS idx_controls_unified_arabic 
ON controls_unified USING gin (to_tsvector('arabic', title_ar || ' ' || COALESCE(description_ar, '')));

-- Partial indexes for active records only
CREATE INDEX IF NOT EXISTS idx_active_controls 
ON controls_unified (control_id, framework_code) WHERE status = 'active';

-- Composite indexes for common queries
CREATE INDEX IF NOT EXISTS idx_framework_authority_lookup 
...
-- Apply trigger to controls_unified (if table exists)
DO $$
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        -- If similarity function doesn't exist, return basic relationships
        RETURN QUERY
        WITH control_data AS (
            SELECT c.control_id, c.title_en, c.description_en, c.category, c.framework_code
            FROM controls_unified c
        ...
        FROM controls_unified c, control_data cd
        WHERE c.control_id != input_control_id
        AND (c.framework_code = cd.framework_code OR c.category = cd.category)
        ORDER BY similarity_score DESC
        LIMIT 10;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT 
        ...
        c.category,
        ts_rank(
            to_tsvector('english', c.title_en || ' ' || COALESCE(c.description_en, '')),
            plainto_tsquery('english', search_query)
        ) as relevance_score,
        ts_headline(
            'english',
            COALESCE(c.description_en, c.title_en),
            plainto_tsquery('english', search_query),
            'MaxWords=20, MinWords=5'
        ) as snippet
    FROM controls_unified c
    WHERE 
        to_tsvector('english', c.title_en || ' ' || COALESCE(c.description_en, '')) @@ plainto_tsquery('english', search_query)
...
FROM controls_unified
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'controls_unified')
...
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'regulatory_authorities_enhanced');
...
1. ⚡ PERFORMANCE
   - Advanced indexing (B-tree, GIN, partial) - Transaction safe
...
5. 🔗 RELATIONSHIPS
   ...
   - Similarity scoring (with fallback)
